...
class AudioClient:
    ...
    def __init__(self, audio_filename, server_address, server_port, RTP_port):
        # basic variables
        self.server_address = server_address
        self.server_port = server_port
        self.audio_port = RTP_port
        ...
        self.RTP_audio_socket = None

        self.current_audio_frame = None

        self.audio_frame_number = 0

    def _print(self, s):
        print('[audio_client.py]: ' + s)
    ...
    def _listen_RTP_audio(self):
        ...
        while True:
            ...
            try:
                ...
                data = self.RTP_audio_socket.recv(1024 * 4)
                ...
                if self.teardown_ack == 1:
                    self.RTP_audio_socket.shutdown(socket.SHUT_RDWR)
        ...
        if session_cseq == self.cseq:
            ...
            if self.session_id == session:
                    if self.last_request_sent == SETUP:
                        ...
                        # Open RTP port.
                        self.RTP_audio_socket = self._open_RTP_port(self.audio_port)
    ...
    def _send_RTSP_request(self, request_type):
        ...
        if request_type == SETUP and self.state == INIT:
            ...
            request = ('SETUP {0} RTSP/1.0\n'
                       'CSeq: {1}\n'
                       'Transport: RTP/AVP;client_port={2}').format(self.audio_filename, self.cseq, self.audio_port)
            ...
            request = ('PLAY {0} RTSP/1.0\n'
                       'CSeq: {1}\n'
                       'Session: {2}').format(self.audio_filename, self.cseq, self.session_id)
